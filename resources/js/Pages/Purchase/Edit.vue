...
watch(search, debounce((value) => {
    router.get(route('purchases.edit', props.purchase.id), {search: value}, {
        preserveState: true,
        replace: true,
    });
}, 300));
...
const form = useForm({
    ...
    status: props.purchase.status || 'cash',
...
onMounted(() => {
    rows.value = props.purchase.purchase_items.map(item => ({
        id: item.product.id,
...
const updateOrder = () => {
    ...
    form.purchase_items = rows.value.map(item => ({
        product_id: item.id,
    ...
    const formData = {
        ...form.data(),
        purchase_items: JSON.stringify(form.purchase_items),

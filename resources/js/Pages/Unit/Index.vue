...
watch(
    search,
    debounce((q) => router.get(route("units.index"), {search: q}, {preserveState: true}), 500)
);
</script>
...
<template>
    ...
    <div class="container mx-auto mt-2 ">
        <div class="flex items-center justify-between  border border-gray-200 rounded-md p-4">
            <h1 class="text-sm lg:text-xl">لیستی پێوانە</h1>
            <div class="flex  items-center gap-2 ">
                <input type="search" placeholder="گەڕان..." class="p-2" v-model="search">
                <Link class="primary-btn flex items-center justify-between gap-2" :href="route('units.create')">
                ...
                <table class="mt-2 text-center">
                    ...
                    <tr v-for="unit in props.units.data" :key="unit.id">
                        <td>{{ unit.id }}</td>
                         <td>{{ unit.name }}</td>
                        <td class="space-x-1">
                            <Link :href="route('units.show',{id:unit.id})" class="table_btn">

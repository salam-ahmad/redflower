...
if (form.customer_id) {
    selectedCustomer.value = props.customers.find(c => c.id === form.customer_id)
    customerSales.value = props.sales.filter(s => s.customer_id === form.customer_id)
}
...
watch(() => form.customer_id, (customerId) => {
    if (customerId) {
        ...
        // Reset sale_id if it doesn't belong to the new customer
        if (form.sale_id && !customerSales.value.find(s => s.id === form.sale_id)) {
            form.sale_id = null
        }
    } else {
...
const submit = () => {
    form.put(route('customer-payments.update', props.payment.id), {
        preserveScroll: true
    })
}
</script>
    ...
    <div class="p-4 container mx-auto space-y-4">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <Link :href="route('customer-payments.show', payment.id)" class="secondary-btn">
                    بینین
                </Link>

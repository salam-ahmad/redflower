...
const deletePayment = () => {
    if (confirm('دڵنیای لە سڕینەوەی ئەم پارەدانە؟')) {
        router.delete(route('customer-payments.destroy', props.payment.id), {
            onSuccess: () => {
                router.visit(route('customer-payments.index'))
            }
        })
    }
}
...
<template>
    <div class="p-4 container mx-auto space-y-4">
        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="bg-white dark:bg-gray-800 border rounded p-6 space-y-4">
                <div>
                    <h2 class="text-lg font-semibold mb-4 border-b pb-2">زانیاری کڕیار</h2>
                </div>

                <div v-if="payment.customer">
                    <div class="space-y-3">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">ناوی کڕیار</p>
                            <Link
                        ...
                        <div v-if="payment.customer.phone">
                            ...
                            <p class="font-medium mt-1">{{ payment.customer.phone }}</p>
                        </div>

                        <div v-if="payment.customer.email">
                            <p class="text-sm text-gray-600 dark:text-gray-400">ئیمەیڵ</p>
                            <p class="font-medium mt-1">{{ payment.customer.email }}</p>
                        </div>

                        <div v-if="payment.customer.address">
                            <p class="text-sm text-gray-600 dark:text-gray-400">ناونیشان</p>
                            <p class="font-medium mt-1">{{ payment.customer.address }}</p>
                        </div>

                        <div v-if="payment.customer.due_amount !== undefined" class="pt-3 border-t">
                            <p class="text-sm text-gray-600 dark:text-gray-400">قەرزی کڕیار</p>
                            <p
                                class="text-xl font-bold mt-1"
                                :class="payment.customer.due_amount > 0 ? 'text-red-600' : 'text-green-600'"
                            >
                                {{ formatNumber(payment.customer.due_amount) }}
                            </p>
                        </div>
                    </div>
                </div>
                <div v-else class="text-gray-500">
                    زانیاری کڕیار بەردەست نییە
                </div>
            </div>
        </div>

        <!-- Sale Reference (if linked to a specific sale) -->
        <div v-if="payment.sale" class="bg-white dark:bg-gray-800 border rounded p-6">
            <h2 class="text-lg font-semibold mb-4 border-b pb-2">پسوڵەی فرۆشتن</h2>
            <div class="grid md:grid-cols-4 gap-4">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">ژمارەی پسوڵە</p>
                    <Link
                        :href="route('sales.show', payment.sale.id)"
                        class="text-lg font-semibold text-blue-600 hover:underline mt-1 inline-block"
                    >
                        #{{ payment.sale.id }}
                    </Link>
                ...
                <div>
                    ...
                    <p class="font-medium mt-1">{{ payment.sale.date }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">کۆی گشتی</p>
                    <p class="font-semibold mt-1">{{ formatNumber(payment.sale.total) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">قەرز</p>
                    <p
                        class="font-semibold mt-1"
                        :class="payment.sale.due_amount > 0 ? 'text-red-600' : 'text-green-600'"
                    >
                        {{ formatNumber(payment.sale.due_amount) }}
                    </p>
                </div>
            </div>
        </div>
